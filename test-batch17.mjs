...
(async () => {
  const { data: stops } = await supabase.from('batch_stops').select('stop_id, batch_id, order_id, sequence_no, location').eq('batch_id', 17);
  ...
  permute(stopPoints);
  console.log('Batch 17 optimal order:');
  bestOrder.forEach((o, i) => console.log(i+1, o.stop.order_id));
})();
